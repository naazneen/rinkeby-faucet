...
async function getBalance(contractAddr, artifact){
	if (typeof window.ethereum != undefined) {
		await requestAccount();

		const faucetContract = getContract(contractAddr, artifact);
		try {
			let balance = await faucetContract.getFaucetBalance();

			// let balance = await balanceResponse.wait();
...
export { getEther, donateEther, getBalance };
